...
class B_CURP{
	...
	var $errores=array();
	private $url0="http://consultas.curp.gob.mx/CurpSP/curp1.do";
	private $url1="http://consultas.curp.gob.mx/CurpSP/curp1.do";
	private $url="";
	private $Modo="GET";
	...
	public function SetDatos($Nombre="", $Ape1="", $Ape2="", $Sexo="", $Dia=0, $Mes=0, $Anio=0, $Entidad=0){
		...
		$Nombre=trim(strtoupper($Nombre));if(preg_match("^[A-ZÑ{$this->Especiales}][A-Z Ñ{$this->Especiales}]+$",$Nombre)===false) $this->errores[]="Error(16): El nombre tiene caracteres no reconocibles. ";//the name have not valid chars 
		$Ape1=trim(strtoupper($Ape1));if(preg_match("^[A-ZÑ{$this->Especiales}][A-Z Ñ{$this->Especiales}]+$",$Ape1)===false) $this->errores[]="Error(17): El apellido tiene caracteres no reconocibles. ";//the first second name have not valid chars 
		$Ape2=trim(strtoupper($Ape2));if(preg_match("^[A-ZÑ{$this->Especiales}][A-Z Ñ{$this->Especiales}]+$",$Ape2)===false) $this->errores[]="Error(18): El apellido 2 tiene caracteres no reconocibles. ";//the second second name have no valid chars 
		if(preg_match("^[HM]$",$Sexo)===false) $this->errores[]="Error(19): el sexo deve ser H/M siendo H para Hombre. ";
	...
	public function SetCURP($xcurp=""){
		...
		$xcurp=trim(strtoupper($xcurp));
		{
			...
			if(preg_match($reg,$xcurp, $regs)===false)$this->errores[]="Error(20): La entidad curp no esta bien estructurada. ";//the state in the curp is not structurated
		...
		$this->CURP=$xcurp;
		$this->vars="strCurp=".urlencode("{$xcurp}")."; strTipo=B"."\r\n";
		if(count($this->errores)) return false;
		return true;
	}
...
echo "<br> host:".$host;
		...
		if(isset($_GET['vars'])&&!preg_match("/\.([^\.]*$)/",$_GET['vars'])){ 
			$vars=$_GET['vars']; 
		}/*else{ 
			$this->errores[]="Error(4): No ha definido vars. ";
		} */
		if(isset($_GET['puerto'])&&!preg_match("/\.([^\.]*$)/",$_GET['puerto'])){ 
			$puerto=$_GET['puerto']; 
		}else{ 
